...
const ms = require("ms");
const prefix = '!'
var mutelirolu = "Siyah"; //MUTELENDİGİ ZAMAN VERİLECEK ROLU  BURAYA YAZINIZ...
...
module.exports.run = async (bot, message, args) => {
  ...
  if (!mutekisi)
    return message.reply(
      `:warning: Lütfen bir kullanıcı etiketleyiniz! \nDoğru Kullanım; **${prefix}rolyasağı <@kullanıcı> <1sn/1dk/1sa/1g>**`
    );
  ...
  message.channel.send(
    ...
    .setFooter("© 2020 Belorya BOT", bot.user.avatarURL())
  );
  bot.guild.cache.get('825911059877330954').channels.cache.get('825919611366146088').send(
    new Discord.MessageEmbed()
    .setThumbnail(message.author.avatarURL())
    .setColor(0x00ae86)
    .setAuthor("İşlem : Mute")
